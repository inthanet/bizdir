...
//************ Category Name Already Exist Check Starts ***************
        ...
        $category_name_exist_check = mysqli_query($conn, "SELECT * FROM " . COUNTRY_PREFIX . "product_categories  WHERE category_name='" . $category_name . "' ");
...
//            $newLink = $link;
//            do {
//                $checkLink = mysqli_query($conn, "SELECT category_id FROM " . COUNTRY_PREFIX . "product_categories WHERE category_slug = '$newLink'");
...
//        $category_slug = checkProductCategorySlug($category_name1);
        ...
        $counter = 1;
        $checkLink = mysqli_query($conn, "SELECT category_id FROM " . COUNTRY_PREFIX . "product_categories WHERE category_slug = '$category_name1'");
        if (mysqli_num_rows($checkLink) > 0) {
        ...
        $sql = mysqli_query($conn, "INSERT INTO  " . COUNTRY_PREFIX . "product_categories (category_name,category_status,category_image,category_filter_pos_id,category_slug,category_cdt)
VALUES ('$category_name','$category_status','$category_image','$category_filter_pos_id', '$category_slug', '$curDate')");
        ...
        $upqry = mysqli_query($conn, "UPDATE " . COUNTRY_PREFIX . "product_categories 
