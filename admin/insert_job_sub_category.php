...
// *********** if Count of sub category name is zero means redirect starts ********
    ...
    if ($cnt == 0) {
        header('Location: admin-add-new-job-sub-category.php');
        exit;
    }
...
//************ sub_category Name Already Exist Check Starts ***************
        ...
        $sub_category_name_exist_check = mysqli_query($conn, "SELECT * FROM " . COUNTRY_PREFIX . "job_sub_categories  WHERE sub_category_name='" . $sub_category_name . "' ");
...
//        if ($i == 0) {
//            function checkJobSubCategorySlug($link, $counter = 1)
//            {
//                global $conn;
//                $newLink = $link;
//                do {
//                    $checkLink = mysqli_query($conn, "SELECT sub_category_id FROM " . COUNTRY_PREFIX . "job_sub_categories WHERE sub_category_slug = '$newLink'");
...
//        if ($i == 0) {
//            $sub_category_slug = checkJobSubCategorySlug($sub_category_name1);
//        } else {
...
//        }
        ...
        $counter = 1;
        $checkLink = mysqli_query($conn, "SELECT sub_category_id FROM " . COUNTRY_PREFIX . "job_sub_categories WHERE sub_category_slug = '$sub_category_name1'");
        if (mysqli_num_rows($checkLink) > 0) {
        ...
        $sql = mysqli_query($conn, "INSERT INTO  " . COUNTRY_PREFIX . "job_sub_categories (sub_category_name,sub_category_status, sub_category_slug,category_id,sub_category_cdt)
VALUES ('$sub_category_name','$sub_category_status', '$sub_category_slug','$category_id','$curDate')");
        ...
        $upqry = mysqli_query($conn, "UPDATE " . COUNTRY_PREFIX . "job_sub_categories 
    ...
    if ($upqry) {
        ...
        header('Location: admin-all-job-sub-category.php');
        exit;
